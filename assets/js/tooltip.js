...
(function() {
  ...
  // --- Toast notification (will be moved into fab-wrapper after FAB is built) ---
  var toastEl = document.createElement('div');
  toastEl.id = 'mode-toast';
  toastEl.className = 'mode-toast';
  toastEl.setAttribute('role', 'status');
  toastEl.setAttribute('aria-live', 'polite');
  document.body.appendChild(toastEl); // temporary, moved below after wrapper is ready
  ...
  // --- Build FAB (inside fixed wrapper to survive browser zoom) ---
  ...
  fabWrapper.appendChild(toastEl);
  fabWrapper.appendChild(fabBtn);
  document.body.appendChild(fabWrapper);

  // --- Sync wrapper to visual viewport (fixes zoom on mobile + desktop) ---
  function syncFabToViewport() {
    if (!window.visualViewport) return;
    var vv = window.visualViewport;
    fabWrapper.style.left   = vv.offsetLeft + 'px';
    fabWrapper.style.top    = vv.offsetTop  + 'px';
    fabWrapper.style.width  = vv.width      + 'px';
    fabWrapper.style.height = vv.height     + 'px';
    // Counter-scale the button so it stays physically fixed-size when zoomed
    fabBtn.style.setProperty('--fab-scale', 1 / vv.scale);
  }
  ...
  // --- Focus mode state (default: reading mode) ---
  ...
  // Default is reading mode (set in HTML class + JS isFocusMode=true)
  // Only switch to study mode if user explicitly chose it before
  if (localStorage.getItem('reading-mode') === 'false') {
    deactivateFocusMode(true);
  } else {
    // Reading mode default — set aria-hidden on page load
    setAriaHidden(true);
  }
